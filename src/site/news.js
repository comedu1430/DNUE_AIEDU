const NEWS_ANNUAL_PROGRAMS = [
  ...
  {
    number: "04",
    title: "Seminar III (Special Lecture)",
    koTitle: "전체 세미나 3(특강)",
    schedule: "October",
    koSchedule: "10월 중",
...
const NEWS_PROJECT_YEARS = ["2026", "2025"];

const NEWS_PROJECTS_BY_YEAR = {
  "2026": NEWS_2026_PROJECTS,
  "2025": NEWS_2025_PROJECTS,
};

const toSortableNewsDate = (value) => {
...
const NEWS_PROJECTS_LATEST_FIRST = NEWS_PROJECT_YEARS.flatMap((year) =>
  (NEWS_PROJECTS_BY_YEAR[year] || []).map((item) => ({ ...item, year }))
).sort((left, right) => toSortableNewsDate(right.date) - toSortableNewsDate(left.date));

const getLocalizedAnnualPrograms = (language) =>
  NEWS_ANNUAL_PROGRAMS.map((item) => ({
    ...item,
...
const getLocalizedNewsProjects = (language) =>
  NEWS_PROJECTS_LATEST_FIRST.map((item) => ({
    ...item,
    displayTitle: language === "ko" ? item.koTitle : item.title,
    displaySummary: language === "ko" ? item.koSummary : item.summary,
  }));

const getLocalizedNewsProjectsByYear = (language, year) =>
  ((NEWS_PROJECTS_BY_YEAR[year] || []).slice().sort(
    (left, right) => toSortableNewsDate(right.date) - toSortableNewsDate(left.date)
  )).map((item) => ({
    ...item,
    year,
    displayTitle: language === "ko" ? item.koTitle : item.title,
